...
/**
 * Paginated chat-list row for the supervisor dashboard (REQ-DASH-2/9).
 * `contactKeyAnon` is the anonymized identifier (SHA-256 of phone || pepper)
 * — the raw phone is never exposed. `openAlertLevel` mirrors the highest
 * open alert for that chat so supervisors can triage at a glance.
 */
export interface DashboardChatSummary {
  ...
  lastActivityAt: string;
  messageCount: number;
  openAlertLevel?: AlertLevel;
}

/**
 * One message of the dual chat view (REQ-DASH-2). Plaintext rows carry
 * `text`; HC-encrypted rows only set `encrypted: true` (decryption is the
 * HC-export path, not the dashboard). Sender stays `user` | `bot`.
 */
...
/**
 * Dual chat view payload (REQ-DASH-2): the session, its user/bot messages
 * and the exact RAG grounding traces (chunks, gate scores, alert level) —
 * the flagged-answer review scenario (orange/yellow).
 */
export interface DashboardChatDetail {
  session: Session;
  messages: DashboardMessage[];
  ragTraces: RagTrace[];
  /** Highest open alert level for the chat, if any. */
  alertLevel?: AlertLevel;
}

/** POST /dashboard/chats/{id}/takeover response (design §3.1, REQ-DASH-3). */
...
/** POST /dashboard/chats/{id}/release response (design §3.1, REQ-DASH-3). */
...
/**
 * Live chat-takeover event pushed over Socket.io (REQ-DASH-3): keeps the
 * supervisor UI in sync with per-chat AI state without polling.
 */
export interface ChatTakeoverEvent {
  sessionId: string;
  aiState: AiState;
  actorId: string;
  occurredAt: string;

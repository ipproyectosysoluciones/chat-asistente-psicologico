...
const envSchema = z
  .object({
    ...
    // Dashboard service (phase 5): JWT access-token TTL (design §3.3,
    // 15-min pilot default) and the dashboard → chat-bot internal path for
    // supervisor takeover replies (POST /api/v1/messages/ingest, REQ-DASH-3).
    // DASHBOARD_CHATBOT_INTERNAL_TOKEN empty = ingest disabled (degraded mode).
    DASHBOARD_JWT_TTL_MINUTES: z.coerce.number().int().positive().default(15),
    DASHBOARD_CHATBOT_BASE_URL: z
      .string()
      .url()
      .default("http://chat-bot:3000"),
    DASHBOARD_CHATBOT_INTERNAL_TOKEN: z.string().default(""),
  })
  .superRefine((data, ctx) => {
    ...
    if (
      data.DASHBOARD_CHATBOT_INTERNAL_TOKEN !== "" &&
      !data.X_INTERNAL_TOKENS.includes(data.DASHBOARD_CHATBOT_INTERNAL_TOKEN)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["DASHBOARD_CHATBOT_INTERNAL_TOKEN"],
        // Never echo the token value (same leak rule as CHATBOT_INTERNAL_TOKEN).
        message:
          "DASHBOARD_CHATBOT_INTERNAL_TOKEN must be one of the tokens in X_INTERNAL_TOKENS when configured",
      });
    }
  });
...
export interface AppConfig {
  ...
  /** Dashboard service wiring (phase 5, REQ-DASH-1/3). */
  dashboard: {
    /** JWT access-token TTL in minutes (design §3.3, 15-min pilot default). */
    jwtTtlMinutes: number;
    /** chat-bot base URL over the private Docker network. */
    chatbotBaseUrl: string;
    /** Token presented to chat-bot's /messages/ingest; "" = ingest disabled. */
    chatbotInternalToken: string;
  };
}
...
export function loadConfig(
  ...
  return {
    ...
    dashboard: {
      jwtTtlMinutes: parsed.DASHBOARD_JWT_TTL_MINUTES,
      chatbotBaseUrl: parsed.DASHBOARD_CHATBOT_BASE_URL,
      chatbotInternalToken: parsed.DASHBOARD_CHATBOT_INTERNAL_TOKEN,
    },
  };
}

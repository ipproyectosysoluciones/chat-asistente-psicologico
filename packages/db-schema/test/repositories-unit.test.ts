...
import { findContactPhoneBySession } from "../src/repositories/contacts";
...
describe("alerts repo (REQ-ALERT-5 dedupe)", () => {
  ...
  it("lists alerts severity-first with total, mapping rows to AlertRow", async () => {
    const { db } = fakeDb([
      {
        rows: [
          {
          ...
          },
          {
            ...
            acknowledged_by: "user-9",
            created_at: new Date("2026-08-14T12:00:00Z"),
            updated_at: new Date("2026-08-14T12:05:00Z"),
            resolved_at: null,
          },
        ],
        rowCount: 2,
    ...
    const page = await listAlerts(db, { limit: 50, offset: 0 });
  ...
  it("orders by severity first and then newest first, with limit/offset params", async () => {
    const { db, sqlTexts, paramLists } = fakeDb([
      { rows: [], rowCount: 0 },
      { rows: [{ total: 0 }], rowCount: 1 },
    ]);
    await listAlerts(db, { limit: 25, offset: 10 });
    const selectSql = sqlTexts[0];
    expect(selectSql).toMatch(/FROM alerts/);
    expect(selectSql).toMatch(/\(level = 'red'\) DESC/);
    expect(selectSql).toMatch(/\(level = 'orange'\) DESC/);
    expect(selectSql).toMatch(/created_at DESC/);
    expect(selectSql).toMatch(/LIMIT \$1 OFFSET \$2/);
    expect(paramLists[0]).toEqual([25, 10]);
  });
});
...
describe("findContactPhoneBySession (task 5.3, REQ-DASH-3)", () => {
  it("returns undefined when the BuilderBot tables are absent (guarded)", async () => {
    const { db, sqlTexts } = fakeDb([{ rows: [{ exists: false }] }]);
    await expect(findContactPhoneBySession(db, "sess-1")).resolves.toBeUndefined();
    expect(sqlTexts).toHaveLength(1);
    expect(sqlTexts[0]).toContain("to_regclass");
  });

  it("resolves the earliest linked contact phone for a session", async () => {
    const { db, sqlTexts, paramLists } = fakeDb([
      { rows: [{ exists: true }] },
      { rows: [{ phone: "+5491100000000" }] },
    ]);
    await expect(
      findContactPhoneBySession(db, "sess-1")
    ).resolves.toBe("+5491100000000");
    expect(sqlTexts[1]).toMatch(/FROM history h\s+JOIN contacts c ON c.id = h.contact_id/);
    expect(sqlTexts[1]).toMatch(/WHERE h.session_id = \$1/);
    expect(paramLists[1]).toEqual(["sess-1"]);
  });

  it("returns undefined when no history row links a contact", async () => {
    const { db } = fakeDb([{ rows: [{ exists: true }] }, { rows: [] }]);
    await expect(findContactPhoneBySession(db, "sess-1")).resolves.toBeUndefined();
  });
});

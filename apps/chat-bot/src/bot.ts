...
export function createBot(pillars: BotPillars, runtime: BotRuntime): Bot {
  ...
  async function runRagLifecycle(effect: { sessionId: string; to: string; message: string }, session: Session): Promise<void> {
    ...
    // Per-chat takeover gate (task 5.3, REQ-DASH-3): while a supervisor holds
    // the session the AI stays OFF locally — no ai-rag call, no emission. The
    // supervisor answers through the /internal/messages/ingest endpoint; the
    // user gets the human-only notice so they know someone is present.
    if (session.aiState === "takeover") {
      runtime.logger.warn("chat: ai disabled by supervisor takeover; human-only", {
        sessionId: effect.sessionId,
      });

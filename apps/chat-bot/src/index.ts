...
const chatDatabase = new PostgresChatDatabase(pool);
...
// Supervisor-reply ingest (task 5.3, REQ-DASH-3): X-Internal-Token gated,
// mounted when the dashboard is configured (DASHBOARD_CHATBOT_INTERNAL_TOKEN
// is cross-checked into X_INTERNAL_TOKENS by the shared config).
const app = createApp({
  logger,
  readiness: {
    database: {
  ...
  ingest: {
    ...
    pillars: { database: chatDatabase, provider },
  },
});

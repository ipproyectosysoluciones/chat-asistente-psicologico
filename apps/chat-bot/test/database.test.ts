...
describe("MemoryChatDatabase (task 4.1 test double)", () => {
  ...
  it("reads back the current session by id (task 5.3)", async () => {
    const db = new MemoryChatDatabase();
    const created = await db.findOrCreateSession("anon-1");
    await expect(db.getSession(created.id)).resolves.toEqual(created);
    await expect(db.getSession("missing")).resolves.toBeUndefined();
  });

  it("resolves the seeded contact phone for supervisor replies (task 5.3)", async () => {
    const db = new MemoryChatDatabase();
    db.seedContactPhone("session-1", "+5491100000000");
    await expect(db.resolveContactPhone("session-1")).resolves.toBe(
      "+5491100000000"
    );
    await expect(db.resolveContactPhone("other")).resolves.toBeUndefined();
  });
});
...
describe("PostgresChatDatabase (task 4.1 adapter over db-schema)", () => {
  ...
  it("delegates getSession to the db-schema repository (task 5.3)", async () => {
    const { db, sqlTexts } = fakeDb([{ rows: [sessionRow()] }]);
    const chatDb = new PostgresChatDatabase(db);
    const session = await chatDb.getSession("session-1");
    expect(session?.id).toBe("session-1");
    expect(sqlTexts.join(" ")).toContain("FROM sessions");
    expect(sqlTexts.join(" ")).toContain("WHERE id = $1");
  });

  it("delegates contact-phone resolution and degrades when BuilderBot tables are absent (task 5.3)", async () => {
    const { db, sqlTexts } = fakeDb([{ rows: [{ exists: false }] }]);
    const chatDb = new PostgresChatDatabase(db);
    await expect(chatDb.resolveContactPhone("session-1")).resolves.toBeUndefined();

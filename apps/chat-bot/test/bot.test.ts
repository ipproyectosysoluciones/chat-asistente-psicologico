...
describe("createBot (task 4.1 orchestrator)", () => {
  ...
  it("never logs the raw phone number", async () => {
    ...
    const failingDatabase = {
      ...
      resolveContactPhone: async () => undefined,
...
describe("createBot — rag message lifecycle (task 4.6, REQ-CHATBOT-2)", () => {
  ...
  it("disables AI emission per-chat under supervisor takeover (task 5.3)", async () => {
    const { bot, provider, db, aiRag } = makeBot({});
    await bot.start();
    ...
    expect(aiRag.process).toHaveBeenCalledTimes(1);
    expect(provider.sentMessages.at(-1)?.text).toBe(AI_DISABLED_TEXT);
    const turns = db.history.map((entry) => entry.text);
    expect(turns).toContain("¿qué es la ansiedad?");
    expect(turns.at(-1)).toBe(AI_DISABLED_TEXT);
  });
